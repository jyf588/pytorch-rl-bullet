...
class DemoEnvironment:
    def __init__(
        ...
        # compute stages
        n_grasp = self.policy_opt.grasp_control_steps #30
        self.n_place = self.policy_opt.place_control_steps #55
        if not self.opt.use_control_skip: #not True = False
            n_grasp *= self.policy_opt.grasp_control_skip #55 * 12
            self.n_place *= self.policy_opt.place_control_skip #8= 6
        self.stage_list = ["reach", "grasp", "transport", "place", "retract"]
        self.duration_list = [
            self.policy_opt.n_reach_steps, #405
            n_grasp, #30
            self.policy_opt.n_transport_steps, #505
            self.n_place, #55
            self.policy_opt.n_retract_steps, #305
        ]
    ...
    def step(self):
        ...
        if self.stage == "plan":
            ...
            if self.stage_ts == self.n_place - 1:
                self.planning_complete = False
    ...
    def plan(self):
        ...
        # Get voice command on the fly
        self.command = get_voice_input()
        if self.command == "stop":
            self.planning_complete = True
            return

        # Reset timestep so stages bo back to the beginning
        self.timestep = 0
