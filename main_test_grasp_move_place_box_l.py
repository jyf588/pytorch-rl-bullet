...
a = InmoovShadowHandPlaceEnvV3(renders=False, is_box=False, is_small=True, place_floor=True, grasp_pi_name=GRASP_PI)
...
for i in range(95):
    ...
    p_obs, reward, done, _ = env_core.step(action)   # TODO:tmp, no normalize
    p_obs = torch.Tensor([p_obs])
    if IS_CUDA:
        p_obs = p_obs.cuda()
