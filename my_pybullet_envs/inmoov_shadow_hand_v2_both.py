...
class InmoovShadowNew:
    def __init__(self,
        ...
        self.fin_actdofs = [9, 10, 11, 14, 15, 16, 19,
                            20, 21, 25, 26, 27, 29, 30, 31, 32, 33]
        self.fin_zerodofs = [8, 13, 18, 24]
        ...
        self.arm_id = self.sim.loadURDF(os.path.join(currentdir,
                                        ...
                                        list(self.base_init_pos), self.sim.getQuaternionFromEuler(
        ...
        for i in range(-1, self.sim.getNumJoints(self.arm_id)):
            self.sim.changeDynamics(
                self.arm_id, i, jointDamping=0.0, linearDamping=0.0, angularDamping=0.0)
            self.sim.changeDynamics(
                self.arm_id_2, i, jointDamping=0.0, linearDamping=0.0, angularDamping=0.0)

        self.scale_mass_inertia(-1, self.ee_id, 0.01)
        self.scale_mass_inertia(
            self.ee_id, self.sim.getNumJoints(self.arm_id), 10.0)
        ...
        self.ll = np.array([self.sim.getJointInfo(self.arm_id, i)[8]
                            for i in range(self.sim.getNumJoints(self.arm_id))])
        self.ul = np.array([self.sim.getJointInfo(self.arm_id, i)[9]
                            for i in range(self.sim.getNumJoints(self.arm_id))])
    ...
    def scale_mass_inertia(self, start, end, ratio=1.0):
        ...
        for i in range(start, end):
            ...
            dyn2 = self.sim.getDynamicsInfo(self.arm_id_2, i)
            mass2 = dyn2[0]
            mass2 = mass2 * ratio
            lid2 = dyn2[2]
            lid2 = (lid2[0] * ratio, lid2[1] * ratio, lid2[2] * ratio,)
            self.sim.changeDynamics(self.arm_id_2, i, mass=mass2)
            self.sim.changeDynamics(
                self.arm_id_2, i, localInertiaDiagonal=lid2)
    ...
    def reset_with_certain_arm_q(self, arm_q):
        ...
        for ind in range(len(self.arm_dofs)):
            self.sim.resetJointState(
                self.arm_id, self.arm_dofs[ind], init_arm_q[ind], 0.0)
            self.sim.resetJointState(
                self.arm_id_2, self.arm_dofs[ind], init_arm_q[ind], 0.0)
        for ind in range(len(self.fin_actdofs)):
            self.sim.resetJointState(
                self.arm_id, self.fin_actdofs[ind], init_fin_q[ind], 0.0)
            self.sim.resetJointState(
                self.arm_id_2, self.fin_actdofs[ind], init_fin_q[ind], 0.0)
        for ind in range(len(self.fin_zerodofs)):
            self.sim.resetJointState(
                self.arm_id, self.fin_zerodofs[ind], 0.0, 0.0)
            self.sim.resetJointState(
                self.arm_id_2, self.fin_zerodofs[ind], 0.0, 0.0)
    ...
    def reset_only_certain_finger_states(self, all_fin_q, tar_act_q):
        ...
        for ind in range(len(self.all_findofs)):
            self.sim.resetJointState(
                self.arm_id, self.all_findofs[ind], all_fin_q[ind], 0.0)
            self.sim.resetJointState(
                self.arm_id_2, self.all_findofs[ind], all_fin_q[ind], 0.0)
    ...
    def reset_with_certain_arm_q_finger_states(self, arm_q, all_fin_q, tar_act_q):
        ...
        for ind in range(len(self.arm_dofs)):
            self.sim.resetJointState(
                self.arm_id, self.arm_dofs[ind], arm_q[ind], 0.0)
            self.sim.resetJointState(
                self.arm_id_2, self.arm_dofs[ind], arm_q[ind], 0.0)
    ...
    def solve_arm_IK(self, w_pos, w_quat, rand_init=False):
        ...
        closeEnough = False
        if rand_init:
            sp = list(self.np_random.uniform(low=-0.03, high=0.03,
                                             size=7)) + [0.0]*len(self.all_findofs)
            sp[3] -= 1.57
        else:
            sp = [-0.44, 0.00, -0.5, -1.8, -0.44, -0.488, -0.8] + \
                [0.0]*len(self.all_findofs)    # dummy init guess IK
        ...
        while not closeEnough and iter < 50:
            for ind in range(len(self.arm_dofs)):
                self.sim.resetJointState(
                    self.arm_id, self.arm_dofs[ind], sp[ind])
                self.sim.resetJointState(
                    self.arm_id_2, self.arm_dofs[ind], sp[ind])
    ...
    def reset(self, w_pos, w_quat, all_fin_q=None, tar_act_q=None):
        ...
        if all_fin_q is None or tar_act_q is None:       # normal reset for grasping
            ...
            while not good_init:
                ...
                # print(cps[0][6])
                if len(cps) == 0:
                    good_init = True   # TODO: init hand last and make sure it does not colllide with env
        else:
            # reset for placing, no collision checking
            self.reset_with_certain_arm_q_finger_states(
                sp, all_fin_q, tar_act_q)
        # input("after reset")
    ...
    def get_cur_jac(self):
        ...
        [jac_t, jac_r] = self.sim.calculateJacobian(self.arm_id, self.ee_id, [0] * 3,
                                                    list(
                                                        wq)+list(self.get_q_dq(self.all_findofs)[0]),
                                                    [0.] * n_dofs, [0.] * n_dofs)
    ...
    def get_norm_diff_tar(self):
        diff_fin = np.array(self.tar_fin_q) - \
            self.get_q_dq(self.fin_actdofs)[0]
    ...
    def get_robot_observation(self, withVel=False, diff_tar=False):
        ...
        # tar pos
        if diff_tar:
            obs.extend(list(np.array(self.tar_arm_q) -
                            self.get_q_dq(self.arm_dofs)[0]))
            obs.extend(list(np.array(self.tar_fin_q) -
                            self.get_q_dq(self.fin_actdofs)[0]))
        else:
    ...
    def apply_action(self, a):
        ...
        self.act = np.array(a)
        # assume arm controls are the first ones.
        self.tar_arm_q += self.act[:len(self.arm_dofs)]
        if self.conservative_clip:
            cur_q, _ = self.get_q_dq(self.arm_dofs)
            self.tar_arm_q = np.clip(
                self.tar_arm_q, cur_q - self.conservative_range, cur_q + self.conservative_range)
        else:
            self.tar_arm_q = np.clip(
                self.tar_arm_q, self.ll[self.arm_dofs], self.ul[self.arm_dofs])
        self.tar_fin_q += self.act[len(self.arm_dofs):]
        self.tar_fin_q = np.clip(
            self.tar_fin_q, self.ll[self.fin_actdofs], self.ul[self.fin_actdofs])
    ...
    def get_link_v_w(self, l_id):
        newLinVel = self.sim.getLinkState(
            self.arm_id, l_id, computeForwardKinematics=1, computeLinkVelocity=1)[6]
        newAngVel = self.sim.getLinkState(
            self.arm_id, l_id, computeForwardKinematics=1, computeLinkVelocity=1)[7]
        return newLinVel, newAngVel

    def get_wrist_wrench(self):
        # TODO: do not know if moments include those produced by force
        joint_reaction = list(
            self.sim.getJointState(self.arm_id, self.ee_id)[2])
        joint_reaction[3] = self.sim.getJointState(self.arm_id, self.ee_id)[
            3]     # TODO: rot axis 1,0,0
        _, p_quat = self.get_link_pos_quat(self.ee_id)
        # transform to world coord
        f_xyz, _ = self.sim.multiplyTransforms(
            [0, 0, 0], p_quat, joint_reaction[:3], [0, 0, 0, 1])
        m_xyz, _ = self.sim.multiplyTransforms(
            [0, 0, 0], p_quat, joint_reaction[3:], [0, 0, 0, 1])
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        arm.reset_with_certain_arm_q(
            [0.02, 0.105-0.2, 0.13, 1.8, -1.57, 0, 0])  # obj [0.2, -0.2, 0]
        ...
        for t in range(400):
            arm.apply_action(arm.np_random.uniform(
                low=-0.003, high=0.003, size=7+17)+np.array([-0.005]*7+[-0.01]*17))
            sim.stepSimulation()
